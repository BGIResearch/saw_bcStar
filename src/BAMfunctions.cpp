...
{ // output CIGAR string
    ...
    if (c->n_cigar > 0)
    {
        ...
        for (uint64_t i = 0; i < c->n_cigar; ++i)
        {
...
void outBAMwriteHeader(BGZF *fp, const string &samh, const vector<string> &chrn, const vector<uint> &chrl)
{
    ...
    if (bgzf_write(fp, (char *)&hlen, sizeof(hlen)) < 0)
    {
        string errMsg = "bgzf_write error";
        sawErrCode(err_fileIO_failed, errMsg);
        cerr << "Error, bgzf_write error" << endl;
    ...
    if (bgzf_write(fp, samh.c_str(), hlen) < 0)
    {
        string errMsg = "bgzf_write error";
        sawErrCode(err_fileIO_failed, errMsg);
        cerr << "Error, bgzf_write error" << endl;
    ...
    if (bgzf_write(fp, (char *)&nchr, sizeof(nchr)) < 0)
    {
        string errMsg = "bgzf_write error";
        sawErrCode(err_fileIO_failed, errMsg);
        cerr << "Error, bgzf_write error" << endl;
    ...
    for (int32 ii = 0; ii < nchr; ii++)
    {
        ...
        if (bgzf_write(fp, (char *)&rlen, sizeof(rlen)) < 0)
        {
            string errMsg = "bgzf_write error";
            sawErrCode(err_fileIO_failed, errMsg);
            cerr << "Error, bgzf_write error" << endl;
        ...
        // this includes \0 at the end of the string
        if (bgzf_write(fp, chrn.at(ii).data(), rlen) < 0)
        {
            string errMsg = "bgzf_write error";
            sawErrCode(err_fileIO_failed, errMsg);
            cerr << "Error, bgzf_write error" << endl;
        ...
        if (bgzf_write(fp, (char *)&slen, sizeof(slen)) < 0)
        {
            string errMsg = "bgzf_write error";
            sawErrCode(err_fileIO_failed, errMsg);
            cerr << "Error, bgzf_write error" << endl;
...
TintType bamAttributeInt(const char *bamAux, const char *attrName)
{ // not tested!!!
