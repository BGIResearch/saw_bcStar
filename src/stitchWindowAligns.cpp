...
void stitchWindowAligns(uint iA, uint nA, int Score, bool WAincl[], uint tR2, uint tG2, Transcript trA, \
	...
	// iA:0 nA：anchor数量 Score：0 WAincl[]：false tR2：0 tG2：0 trA：转录本 Lread：read长度 WA：window中anchor的信息 R：read序列
	// wTr 窗口中所有的转录本
    ...
    if (iA>=nA) {//no more aligns to add, finalize the transcript
        ...
        if (P.outFilterIntronMotifs=="None") {//no filtering
            ...
            errOut << "SOLUTION: re-run BCSTAR with --outFilterIntronMotifs = None -OR- RemoveNoncanonical -OR- RemoveNoncanonicalUnannotated\n";
