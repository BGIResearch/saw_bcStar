...
int bamAttrArrayWrite(uint32_t attr, const char* tagName, char* attrArray ) {
  attrArray[0]=tagName[0];attrArray[1]=tagName[1];
  attrArray[2]='i';
  *( (uint32_t*) (attrArray+3))=attr;
...
int bamAttrArrayWrite(const vector<uint32_t> &attr, const char* tagName, char* attrArray ) {
  attrArray[0]=tagName[0];attrArray[1]=tagName[1];
  attrArray[2]='B';
  attrArray[3]='I';
  *( (uint32_t*) (attrArray+4))=attr.size();
  memcpy(attrArray+4+sizeof(uint32_t),attr.data(),sizeof(uint32_t)*attr.size());//copy array data
  return 4+sizeof(uint32_t)+sizeof(uint32_t)*attr.size();
};
        ...
        if (alignType>=0) {//this mate is unmapped
            ...
            for (uint ii=0;ii<outSAMattrOrder.size();ii++) {
                switch (outSAMattrOrder[ii]) {
                  	case ATTR_Cx:
					  	attrN+=bamAttrArrayWrite(trOut.corX,"Cx",attrOutArray+attrN);break;
					case ATTR_Cy:
					  	attrN+=bamAttrArrayWrite(trOut.corY,"Cy",attrOutArray+attrN);break;
                    case ATTR_NH:
...
//                            attrN+=bamAttrArrayWrite(soloRead->readBar->umiSeq,"UR",attrOutArray+attrN);
					  string tmpUmi = trOut.umi;
					  attrN += bamAttrArrayWrite(tmpUmi, "UR", attrOutArray + attrN);
					  break;
					}
                    case ATTR_UY:
