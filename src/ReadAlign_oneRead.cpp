...
int ReadAlign::oneRead()
{ // process one read: load, map, write
    ...
    readStatus[0] = readLoad(*(readInStream[0]), P, 0, readLength[0], readLengthOriginal[0], readNameMates[0], Read0[0], Read1[0], Qual0[0], Qual1[0], clip3pNtotal[0], clip5pNtotal[0], clip3pAdapterN[0], iReadAll, readFilesIndex, readFilter, readNameExtra[0], coorX, coorY, umi);
    if (P.readNmates == 2)
    { // read the 2nd mate
        readStatus[1] = readLoad(*(readInStream[1]), P, 1, readLength[1], readLengthOriginal[1], readNameMates[1], Read0[1], Read1[0] + readLength[0] + 1, Qual0[1], Qual1[0] + readLength[0] + 1, clip3pNtotal[1], clip5pNtotal[1], clip3pAdapterN[1], iReadAll, readFilesIndex, readFilter, readNameExtra[1], coorX, coorY, umi);
        ...
        if (Lread > DEF_readSeqLengthMax)
        {
            ...
            errOut << "SOLUTION: increase DEF_readSeqLengthMax in IncludeDefine.h and re-compile BCSTAR" << endl
                   << flush;
            sawErrCode(err_sw_exception, errOut.str());
            exitWithError(errOut.str(), std::cerr, P.inOut->logMain, EXIT_CODE_INPUT_FILES, P);
        };
    ...
    if (!peOv.yes)
    { // if the alignment was not mates merged - otherwise the chimeric detection was already done
        chimericDetection();
    };

    if (P.pCh.out.bam && chimRecord)
    { // chimeric alignment was recorded in main BAM files, and it contains the representative portion, so non-chimeric aligmnent is not output
        return 0;
    };
