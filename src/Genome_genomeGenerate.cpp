...
void Genome::genomeGenerate()
{
    ...
    if (sjdbOverhang <= 0 && (pGe.sjdbFileChrStartEnd.at(0) != "-" || pGe.sjdbGTFfile != "-"))
    {
        ...
        string errMsg = "EXITING because of FATAL INPUT PARAMETER ERROR: for generating genome with annotations (--sjdbFileChrStartEnd or --sjdbGTFfile options)\n";
        errMsg += "you need to specify >0 --sjdbOverhang\n";
        errMsg += "SOLUTION: re-run genome generation specifying non-zero --sjdbOverhang, which ideally should be equal to OneMateLength-1, or could be chosen generically as ~100\n";
    ...
    if (pGe.sjdbFileChrStartEnd.at(0) == "-" && pGe.sjdbGTFfile == "-")
    {
        ...
        {
            ostringstream errOut;
            string errMsg = "EXITING because of FATAL INPUT PARAMETER ERROR: when generating genome without annotations (--sjdbFileChrStartEnd or --sjdbGTFfile options)\n";
            errMsg += "do not specify >0 --sjdbOverhang\n";
            errMsg += "SOLUTION: re-run genome generation without --sjdbOverhang option\n";
            sawErrCode(err_param_invalid_STAR, errMsg);
    ...
    *P.inOut->logStdOut << timeMonthDayTime(rawTime) << " ... starting to generate Genome files\n"
    ...
    if (P.limitGenomeGenerateRAM < (nG1alloc + nG1alloc / 3))
    { // allocate nG1alloc/3 for SA generation
        ostringstream errOut;
    ...
    *P.inOut->logStdOut << timeMonthDayTime(rawTime) << " ... starting to sort Suffix Array. This may take a long time...\n"
    ...
    { // sort SA chunks
        ...
        for (uint ii = 0; ii < N; ii++)
        { // re-fill the array backwards for sorting
            swap(G[N2 - 1 - ii], G[ii]);
        };
        globalG = G;
        ...
        if (P.runThreadN > 1)
            saChunkSize = min(saChunkSize, nSA / (P.runThreadN - 1));
...
#pragma omp parallel for num_threads(P.runThreadN) ordered schedule(dynamic, 1)
        ...
        {                                                        // start the chunk cycle: sort each chunk with qsort and write to a file
            uint *saChunk = new uint[indPrefChunkCount[iChunk]]; // allocate local array for each chunk
            for (uint ii = 0, jj = 0; ii < N2; ii += pGe.gSAsparseD)
            { // fill the chunk with SA indices
                if (G[ii] < 4)
                {
                    uint p1 = (G[ii] << 12) + (G[ii - 1] << 8) + (G[ii - 2] << 4) + G[ii - 3];
...
#define SA_CHUNK_BLOCK_SIZE 10000000
        ...
        { // load files one by one and convert to packed
            ...
            while (!saChunkFile.eof())
            { // read blocks from each file
        ...
        { //
            ostringstream errOut;
            string errMsg = "EXITING because of FATAL problem while generating the suffix array\n";
            errMsg += "The number of indices read from chunks = " + to_string(packedInd) + " is not equal to expected nSA=" + to_string(nSA) + "\n";
            errMsg += "SOLUTION: try to re-run suffix array generation, if it still does not work, report this problem to the author\n";
            sawErrCode(err_param_invalid_STAR, errMsg);
        ...
        for (uint ii = 0; ii < N; ii++)
        { // return to normal order for future use
            swap(G[N2 - 1 - ii], G[ii]);
        };
        delete[] indPrefCount;
        delete[] indPrefStart;
