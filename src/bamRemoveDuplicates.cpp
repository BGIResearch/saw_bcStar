...
{ // calculates align start extending right S operation
    uint32 *cig = (uint32 *)(((char *)p) + 9 * 4 + ((p[3] << 24) >> 24));
    if (((cig[0] << 28) >> 28) == 4)
    { // first (right) operation is S
        return p[2] - (cig[0] >> 4);
    }
    else
    {
...
uint32 funCigarExtendS(const uint32 *const p, uint32 *cout)
{
    ...
    if (((cig[n - 1] << 28) >> 28) == 4)
    { // remove last S opeartion add length to previous M
        --n1;
...
void bamRemoveDuplicates(const string bamFileName, const string bamFileNameOut, Parameters &P)
{
    ...
    if (bam_hdr_write(bgzfOut, bamHeader) < 0)
    {
        string errMsg = "bam_hdr_write error";
        sawErrCode(err_fileIO_failed, errMsg);
        cerr << "Error, bam_hdr_write error" << endl;
        exit(1);
    }

    uint bamLengthMax = P.limitBAMsortRAM; // max length to load
    if (bamLengthMax == 0)
        bamLengthMax = 16000000000;
    ...
    {
        if (bamE1 > bamLength)
        { // reached end of loaded BAM block, add BAM data
            if (bamLength < bamLengthMax && bamLength > 0)
            { // reached end of BAM file, cannot load more
                bamFileEnd = true;
            }
            else
            {
                if (bamS == 0 && bamLength > 0)
                { // TODO
                    ostringstream errOut;
                    string errMsg = "EXITING because of fatal ERROR: not enough memory for marking duplicates \n";
                    errMsg += "SOLUTION: re-run BCSTAR with at least --limitBAMsortRAM " + to_string(bamLengthMax * 2);
                    sawErrCode(err_resource_oom, errMsg);
                    errOut << "EXITING because of fatal ERROR: not enough memory for marking duplicates \n";
                    errOut << "SOLUTION: re-run BCSTAR with at least --limitBAMsortRAM " << bamLengthMax * 2;
                    exitWithError(errOut.str(), std::cerr, P.inOut->logMain, EXIT_CODE_PARAMETER, P);
                };

                // write out processed block
                if (bgzf_write(bgzfOut, bamRaw, bamS) < 0)
                {
        ...
        if (chrE != chrS || (rightMax > 0 && leftE > rightMax) || bamFileEnd)
        { // found new group of reads to be processed, start collapsing procedure
            ...
            for (uint pp = 0; pp < grN / 2; pp++)
            {
                ...
                if (pp == (grN / 2 - 1) || funCompareCoordFlagCigarSeq((void *)(aD + pp * 2), (void *)(aD + pp * 2 + 2)) != 0)
                { // next pair is not equal to the current one
                    // un-mark duplicates
            ...
            if (bamFileEnd)
                break; // exit the main cycle over blocks
            rightMax = 0;
            bamS = bamE;
            grN = 0;
        };
        ...
        if (nMult == 1)
        { // record this alignment in the current group, unique mappers only. Multi-mappers will not be considered for collapsing, and will remain marked as duplicates
            if (grN >= grNmax)
            { // reallocate
                grNmax = grN * 2;
            ...
            ++grN;
            if (rightE > leftE)
            { // left mate, record coordinate of its right mate
    ...
    if (bgzf_write(bgzfOut, bamRaw, bamLength) < 0)
    {
        string errMsg = "bgzf_write error";
        sawErrCode(err_fileIO_failed, errMsg);
        cerr << "Error, bgzf_write error" << endl;
