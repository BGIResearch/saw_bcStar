...
void FastqReader::readToBuf()
{
	if (mZipped)
	{
		...
		if (mBufDataLen == -1)
		{
			string errMsg = "failed to read gzip file:" + mFilename;
...
{
	if (ends_with(mFilename, ".gz") || ends_with(mFilename, ".bgz"))
	{
	...
	else
	{
		...
		if (mFile == NULL)
		{
			string errMsg = "failed to open file: " + mFilename;
...
Read *FastqReader::read()
{
	...
	else
	{
		...
		{
			string errMsg = "sequence and quality have different length";
...
bool FastqReader::isZipFastq(string filename)
{
	if (ends_with(filename, ".fastq.gz"))
		...
		return true;
	else if (ends_with(filename, ".bgz"))
	{
		return true;
	}
	return false;
...
FastqReaderPair::FastqReaderPair(string leftName, bool hasQuality, bool phred64)
{
	mLeft = new FastqReader(leftName, hasQuality, phred64);
	mRight = nullptr;
}
ReadPair *FastqReaderPair::readR1()
{
	Read *l = mLeft->read();
	Read *r = NULL;
	if (!l)
