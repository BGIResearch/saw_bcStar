...
{ // insert new sequences into the SA
    ...
    if (GstrandBit1 < 32)
        GstrandBit1 = 32; // TODO: use simple access function for SA
    if (GstrandBit1 + 1 != SA.wordLength)
    { // sequence is too long - GstrandBit changed
        ostringstream errOut;
        string errMsg = "EXITING because of FATAL ERROR: cannot insert sequence on the fly because of strand GstrandBit problem\n";
        ...
        // errOut << "SOLUTION: please contact BCSTAR author at https://groups.google.com/forum/#!forum/rna-star\n";
...
#pragma omp for schedule(dynamic, 1000)
    for (uint ii = 0; ii < 2 * nG1; ii++)
    { // find insertion points for each of the sequences
    ...
    for (uint ii = 0; ii < 2 * nG1; ii++)
    { // remove entries that cannot be inserted, this cannot be done in the parallel cycle above
    ...
    */
    ...
    {
        while (isa == indArray[isa1 * 2])
        { // insert new index before the existing index
