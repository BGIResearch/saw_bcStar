...
{ // map one chunk. Input reads stream has to be setup in RA->readInStream[ii]
    RA->statsRA.resetN();
    ...
    int iter = 0;
    while (readStatus == 0)
    { // main cycle over all reads

        readStatus = RA->oneRead(); // map one read
        iter++;
        if (iter < 2)
        {
            //            cout<<get_local_time()<<"...thread "<<iThread<<". read1 name "<<RA->readNameMates[0]<<endl;
        }
        if (iter % 10000 == 0)
        {
            //            cout<<get_local_time()<<"...thread "<<iThread<<". mapped reads number: "<<iter<<endl;
        }
        if (readStatus == 0)
        { // there was a read processed
            RA->iRead++;
        ...
        if (P.outSAMbool)
        {
            ...
            { // write buffer to disk because it's almost full, or all reads are mapped
                if (P.outSAMorder == "PairedKeepInputOrder" && P.runThreadN > 1)
                { // output chunks into separate files
