...
void Parameters::openReadsFiles()
{
    ...
    {
        string errMsg = "no input files";
        sawErrCode(err_param_unset_STAR, errMsg);
    ...
    { // read from file
        for (uint ii = 0; ii < readFilesIn.size(); ii++)
        {                                // open readIn files
    ...
    else if (readFilesCommandString.find("gzip") != string::npos || readFilesIn[0].rfind(".gz") == readFilesIn.size() - 3)
    {
        // added by gc
        // replace fifo with gzFile
        for (uint ii = 0; ii < readFilesIn.size(); ii++)
        {
            inOut->gzReadIn[ii] = gzopen(readFilesIn[ii].c_str(), "r");
            if (inOut->gzReadIn[ii] == NULL)
            {
                string errMsg = "fail to open file," + readFilesIn[ii];
                sawErrCode(err_fileOpen_failed, errMsg);
                cerr << "Error, fail to open file," << readFilesIn[ii] << endl;
                exit(1);
            }
        }
    }
    ...
    { // create fifo files, execute pre-processing command
        ...
        for (volatile uint imate = 0; imate < readFilesIn.size(); imate++)
        { // open readIn files
    ...
    if (readFilesTypeN == 10)
    { // SAM file - skip header lines
