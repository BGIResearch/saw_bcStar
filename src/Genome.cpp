...
void Genome::freeMemory()
{ // free big chunks of memory used by genome and suffix array

    if (pGe.gLoad == "NoSharedMemory")
    { // can deallocate only for non-shared memory
        delete[] G1;
        G1 = NULL;
        SA.deallocateArray();
...
uint Genome::OpenStream(string name, ifstream &stream, uint size)
{
    ...
    if (!stream.good())
    {
        ...
        string errMsg = "EXITING because of FATAL ERROR: could not open genome file: " + pGe.gDir + "/" + name + "\n";
        errMsg += "SOLUTION: check that the path to genome files, specified in --genomeDir is correct and the files are present, and have user read permissions\n";
    ...
    else
    {
        ...
        {
            ostringstream errOut;
            string errMsg = "EXITING because of FATAL ERROR: failed reading from genome file: " + pGe.gDir + "/" + name + "\n";
            errMsg += "SOLUTION: re-generate the genome index\n";
            sawErrCode(err_fileOpen_failed, errMsg);
...
{ // allocate and load Genome
    ...
    *(P.inOut->logStdOut) << timeMonthDayTime(rawtime) << " ..... loading genome\n"
    ...
    {
        ostringstream errOut;
        string errMsg = "EXITING because of FATAL ERROR: could not open genome file " + pGe.gDir + "/genomeParameters.txt";
        errMsg + "SOLUTION: check that the path to genome files, specified in --genomeDir is correct and the files are present, and have user read permsissions\n";
        sawErrCode(err_fileOpen_failed, errMsg);
    ...
    { //
        ostringstream errOut;
        string errMsg = "EXITING because of FATAL ERROR: read no value for the versionGenome parameter from genomeParameters.txt file\n";
        errMsg += "SOLUTION: please re-generate genome from scratch with the latest version of BCSTAR\n";
        sawErrCode(err_refLoad_failed, errMsg);
        errOut << "EXITING because of FATAL ERROR: read no value for the versionGenome parameter from genomeParameters.txt file\n";
        errOut << "SOLUTION: please re-generate genome from scratch with the latest version of BCSTAR\n";
        exitWithError(errOut.str(), std::cerr, P.inOut->logMain, EXIT_CODE_GENOME_FILES, P);
    }
    else if (P1.versionGenome == P.versionGenome)
    { //
        P.inOut->logMain << "Genome version is compatible with current BCSTAR\n";
    }
    else
    {
        ostringstream errOut;
        string errMsg = "EXITING because of FATAL ERROR: Genome version: " + P1.versionGenome + " is INCOMPATIBLE with running BCSTAR version: " + STAR_VERSION + "\n";
        errMsg += "SOLUTION: please re-generate genome from scratch with running version of BCSTAR, or with version: " + P.versionGenome + "\n";
        sawErrCode(err_refLoad_failed, errMsg);
        errOut << "EXITING because of FATAL ERROR: Genome version: " << P1.versionGenome << " is INCOMPATIBLE with running BCSTAR version: " << STAR_VERSION << "\n";
        errOut << "SOLUTION: please re-generate genome from scratch with running version of BCSTAR, or with version: " << P.versionGenome << "\n";
        exitWithError(errOut.str(), std::cerr, P.inOut->logMain, EXIT_CODE_GENOME_FILES, P);
    };
    ...
    { // if sjdbInsert, and genome had junctions, and genome is old - it should be re-generated with new BCSTAR
        ostringstream errOut;
        string errMsg = "EXITING because of FATAL ERROR: old Genome is INCOMPATIBLE with on the fly junction insertion\n";
        errMsg += "SOLUTION: please re-generate genome from scratch with the latest version of BCSTAR\n";
        sawErrCode(err_refLoad_failed, errMsg);
    ...
    { // if pGe.sjdbOverhang was defined at the genome generation step,the mapping step value has to agree with it
        ostringstream errOut;
        string errMsg = "EXITING because of fatal PARAMETERS error: present --sjdbOverhang=" + to_string(pGe.sjdbOverhang) + " is not equal to the value at the genome generation step =" + to_string(P1.pGe.sjdbOverhang) + "\n";
        sawErrCode(err_param_invalid_STAR, errMsg);
        ...
        sharedMemory == NULL)
    {
        ...
        try
        {
            ...
            if (pGe.gLoad == "Remove")
            { // kill the genome and exit
                if (sharedMemory->NeedsAllocation())
                { // did not find genome in shared memory, nothing to kill
                    ostringstream errOut;
                    string errMsg = "EXITING: Did not find the genome in memory, did not remove any genomes from shared memory\n";
        ...
        if (!sharedMemory->IsAllocator())
        {
            ...
            while (*shmNG != nGenome)
            {
                ...
                {
                    ostringstream errOut;
                    string errMsg = "load reference failed";
                    sawErrCode(err_refLoad_failed, errMsg);
                    errOut << "EXITING because of FATAL ERROR: waited too long for the other job to finish loading the genome" << strerror(errno) << "\n"
                           << flush;
                    errOut << "SOLUTION: remove the shared memory chunk by running BCSTAR with --genomeLoad Remove, and restart BCSTAR" << flush;
    ...
    else if (pGe.gLoad == "NoSharedMemory")
    { // simply allocate memory, do not use shared memory
        genomeInsertL = 0;
    ...
    //         errOut << "SOLUTION: check shared memory settings as explained in BCSTAR manual, OR run BCSTAR with --genomeLoad NoSharedMemory to avoid using shared memory\n" <<flush;
    ...
    if (pGe.gLoad == "NoSharedMemory" || isAllocatorProcess)
    { // load genome and SAs from files
        // load genome
        ...
        { // attach a tail with the largest symbol
            G1[ii] = K - 1;
            G[nGenome + ii] = K - 1;
        };
...
#ifdef COMPILE_FOR_MAC
    {
        ...
        for (uint ii = 0; ii < nGenome; ii++)
            sum1 += (uint)(unsigned char)G[ii];
        P.inOut->logMain << "Sum of all Genome bytes: " << sum1 << "\n"
                         << flush;
        sum1 = 0;
        for (uint ii = 0; ii < SA.lengthByte; ii++)
            sum1 += (uint)(unsigned char)SA.charArray[ii];
        P.inOut->logMain << "Sum of all SA bytes: " << sum1 << "\n"
                         << flush;
        sum1 = 0;
        for (uint ii = 0; ii < SAi.lengthByte; ii++)
            sum1 += (uint)(unsigned char)SAi.charArray[ii];
    ...
    if (nGenome == chrStart[nChrReal])
    { // no sjdb
        sjdbN = 0;
        sjGstart = chrStart[nChrReal] + 1; // not sure why I need that
    }
    else
    { // there are sjdb chromosomes
        ifstream sjdbInfo((pGe.gDir + "/sjdbInfo.txt").c_str());
        if (sjdbInfo.fail())
        {
            ostringstream errOut;
            string errMsg = "EXITING because of FATAL error, could not open file " + (pGe.gDir + "/sjdbInfo.txt") + "\n";
            errMsg += "SOLUTION: check that the path to genome files, specified in --genomeDir is correct and the files are present, and have user read permsissions\n";
            sawErrCode(err_fileOpen_failed, errMsg);
        ...
        { // get the info about junctions from sjdbInfo.txt
            {
                uint16 d1, d2, d3, d4;
            ...
            if (sjdbMotif[ii] == 0)
            { // shinon-canonical junctions back to their true coordinates
    ...
    {
        P.inOut->logMain << "winBinNbits=" << P.winBinNbits << " > "
                         << "pGe.gChrBinNbits=" << pGe.gChrBinNbits << "   redefining:\n";
...
void Genome::HandleSharedMemoryException(const SharedMemoryException &exc, uint64 shmSize)
{
    ...
    {
    case EOPENFAILED:
        errOut << "EXITING because of FATAL ERROR: problems with shared memory: error from shmget() or shm_open()." << endl
               << flush;
        errOut << "SOLUTION: check shared memory settings as explained in BCSTAR manual, OR run BCSTAR with --genomeLoad NoSharedMemory to avoid using shared memory" << endl
               << flush;
        break;
    case EEXISTS:
        ...
        errOut << "Possible cause 3: allowed shared memory size is not large enough. SOLUTIONS: (i) consult BCSTAR manual on how to increase shared memory allocation; "
                  "(ii) ask your system administrator to increase shared memory allocation; (iii) run BCSTAR with --genomeLoad NoSharedMemory"
               << endl
               << flush;
        break;
    case EFTRUNCATE:
        ...
        break;
    case EMAPFAILED:
        errOut << "EXITING because of FATAL ERROR: problems with shared memory: error from shmat() while trying to get address of the shared memory piece." << endl
               << flush;
        errOut << "SOLUTION: check shared memory settings as explained in BCSTAR manual, OR run BCSTAR with --genomeLoad NoSharedMemory to avoid using shared memory" << endl
               << flush;
        break;
    case ECLOSE:
        ...
        errOut << "EXITING because of FATAL ERROR: could not close the shared memory object." << endl
               << flush;
        break;
    case EUNLINK:
        ...
        string errMsg = "load reference failed";
        sawErrCode(err_refLoad_failed, errMsg);
        errOut << "EXITING because of FATAL ERROR:  could not delete the shared object." << endl
               << flush;
#else
        errOut << "EXITING because of FATAL ERROR: problems with shared memory: error from shmctl() while trying to remove shared memory piece." << endl
               << flush;
        errOut << "SOLUTION: check shared memory settings as explained in BCSTAR manual, OR run BCSTAR with --genomeLoad NoSharedMemory to avoid using shared memory" << endl
               << flush;
#endif
        break;
    default:
        errOut << "EXITING because of FATAL ERROR: There was an issue with the shared memory allocation. Try running BCSTAR with --genomeLoad NoSharedMemory to avoid using shared memory.";
        break;
    }
...
//////////////////////////////////////////////////////////////////////////////////////////
void Genome::chrInfoLoad()
{ // find chrStart,Length,nChr from Genome G
    ...
    {
        ostringstream errOut;
        string errMsg = "EXITING because of FATAL error, could not open file " + (pGe.gDir + "/chrName.txt") + "\n";
        errMsg += "SOLUTION: re-generate genome files with BCSTAR --runMode genomeGenerate\n";
        sawErrCode(err_fileOpen_failed, errMsg);
        errOut << "EXITING because of FATAL error, could not open file " << (pGe.gDir + "/chrName.txt") << "\n";
        errOut << "SOLUTION: re-generate genome files with BCSTAR --runMode genomeGenerate\n";
        exitWithError(errOut.str(), std::cerr, P.inOut->logMain, EXIT_CODE_INPUT_FILES, P);
    };
    ...
    {
        ostringstream errOut;
        string errMsg = "EXITING because of FATAL error, could not open file " + (pGe.gDir + "/chrLength.txt") + "\n";
        errMsg += "SOLUTION: re-generate genome files with BCSTAR --runMode genomeGenerate\n";
        sawErrCode(err_fileOpen_failed, errMsg);
    ...
    {
        ostringstream errOut;
        string errMsg = "EXITING because of FATAL error, could not open file " + (pGe.gDir + "/chrStart.txt") + "\n";
        errMsg += "SOLUTION: re-generate genome files with BCSTAR --runMode genomeGenerate\n";
        sawErrCode(err_fileOpen_failed, errMsg);
...
void Genome::chrBinFill()
{
    ...
    {
        if (ii * genomeChrBinNbases >= chrStart[ichr])
            ichr++;
        chrBin[ii] = ichr - 1;
    };
};
