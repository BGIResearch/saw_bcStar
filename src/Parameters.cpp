...
void Parameters::inputParameters(int argInN, char *argIn[])
{ // input parameters: default, from files, from command line
...
#include "parametersDefault.xxd"
    ...
    { // scan parameters from command line
        ...
        for (int iarg = 1; iarg < argInN; iarg++)
        {
            ...
            if (found != string::npos && oneArg.substr(0, 2) == "--")
            { // --parameter=value
                string key = oneArg.substr(2, found - 2);
                string val = oneArg.substr(found + 1);
                if (val.find_first_of(" \t") != std::string::npos)
                { // there is white space in the argument, put "" around
                    val = '\"' + val + '\"';
                };
                commandLineFile += '\n' + key + ' ' + val;
            }
            else if (oneArg.substr(0, 2) == "--")
            { // parameter name, cut --
                commandLineFile += '\n' + oneArg.substr(2);
            }
            else
            { // parameter value
                if (oneArg.find_first_of(" \t") != std::string::npos)
                { // there is white space in the argument, put "" around
                    oneArg = '\"' + oneArg + '\"';
                };
    ...
    inOut->logMain << "BCSTAR version=" << STAR_VERSION << "\n";
    //    inOut->logMain << "BCSTAR compilation time,server,dir=" << COMPILATION_TIME_PLACE << "\n";
    ...
    */
    ...
    if (parametersFiles.at(0) != "-")
    { // read parameters from a user-defined file
    ...
    if (argInN > 1)
    { // scan all parameters from command line and override previuos values
    ...
    */
    ...
    else
    {
        ...
        sawErrCode(err_param_invalid_STAR, errOut.str());
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    if (outTmpDir == "-")
    {
        ...
        //		if(access(outFileTmp.c_str(),F_OK)==0){
        //		  string cmd = "rm -rf " + P.outFileTmp;
        //		  if(system(cmd.c_str())==-1) {
        //			cerr<<"Error code: "<<sawErrCode(168)<<endl;
    ...
    if (mkdir(outFileTmp.c_str(), runDirPerm) != 0)
    {
        ...
        errOut << "SOLUTION: (i) please check the path and writing permissions \n (ii) if you specified --outTmpDir, and this directory exists - please remove it before running BCSTAR\n"
               << flush;
        sawErrCode(err_fileOpen_failed, errOut.str());
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    outBAMcoord = false;
    if (runMode == "alignReads" && outSAMmode != "None")
    { // open SAM file and write header
        if (outSAMtype.at(0) == "BAM")
        {
            if (outSAMtype.size() < 2)
            {
                ...
                errOut << "SOLUTION: re-run BCSTAR with one of the allowed values of --outSAMtype BAM Unsorted OR SortedByCoordinate OR both\n";
                exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
            };
        ...
        else if (outSAMtype.at(0) == "SAM")
        {
            if (outSAMtype.size() > 1)
            {
                ...
                errOut << "SOLUTION: re-run BCSTAR with with '--outSAMtype SAM' only, or with --outSAMtype BAM Unsorted|SortedByCoordinate\n";
                exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
            };
            outSAMbool = true;
            if (outStd == "SAM")
        ...
        else
        {
            ...
            errOut << "SOLUTION: re-run BCSTAR with one of the allowed values of outSAMtype: BAM or SAM \n"
                   << flush;
            exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
        };
    };
    ...
    if (!outBAMcoord && outWigFlags.yes && runMode == "alignReads")
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with with --outSAMtype BAM SortedByCoordinate, or, id you also need unsroted BAM, with --outSAMtype BAM SortedByCoordinate Unsorted\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    for (uint ii = 0; ii < 1; ii++)
    {
        if (parArray[ii]->inputLevel > 0)
        {
            ...
            errOut << "SOLUTION: please remove this parameter from the command line or input files and re-start BCSTAR\n"
                   << flush;
            exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
        };
    };
    ...
    if (outFilterType == "BySJout" && outSAMorder == "PairedKeepInputOrder")
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR without setting one of those parameters.\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    if (!outSAMbool && outSAMorder == "PairedKeepInputOrder")
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR without --outSAMorder=PairedKeepInputOrder, or with --outSAMorder=PairedKeepInputOrder --outSMAtype SAM .\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    // SJ filtering
    for (int ii = 0; ii < 4; ii++)
    {
        if (outSJfilterOverhangMin.at(ii) < 0)
            outSJfilterOverhangMin.at(ii) = numeric_limits<int32>::max();
        if (outSJfilterCountUniqueMin.at(ii) < 0)
            outSJfilterCountUniqueMin.at(ii) = numeric_limits<int32>::max();
        if (outSJfilterCountTotalMin.at(ii) < 0)
            outSJfilterCountTotalMin.at(ii) = numeric_limits<int32>::max();
        if (outSJfilterDistToOtherSJmin.at(ii) < 0)
            outSJfilterDistToOtherSJmin.at(ii) = numeric_limits<int32>::max();

        if (alignSJstitchMismatchNmax.at(ii) < 0)
            alignSJstitchMismatchNmax.at(ii) = numeric_limits<int32>::max();
    };

    if (limitGenomeGenerateRAM == 0)
    { // must be >0
    ...
    { // read groups
        ...
        {
            string linefull;
            for (uint ii = 0; ii < outSAMattrRGline.size(); ii++)
            { // concatenate into one line
                if (ii == 0 || outSAMattrRGline.at(ii) == ",")
                { // start new entry
                    if (ii > 0)
                        ++ii;                                                 // skip comma
                    ...
                    if (outSAMattrRGlineSplit.back().substr(0, 3) != "ID:")
                    {
                        ...
                        errOut << "SOLUTION: re-run BCSTAR with all lines in --outSAMattrRGline starting with ID:xxx\n";
                        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
                    };
                    outSAMattrRG.push_back(outSAMattrRGlineSplit.back().substr(3)); // this adds the ID field
                }
                else
                { // keep adding fields to this RG line, until the next comma
    ...
    else if (readFilesTypeN == 10)
    { // find the number of mates from the SAM file
    ...
    { // 2-pass parameters
        ...
        if (pGe.gLoad != "NoSharedMemory")
        {
            ...
            errOut << "SOLUTION: re-run BCSTAR with --genomeLoad NoSharedMemory ; this is the only option compatible with --twopassMode Basic .\n";
            sawErrCode(err_param_invalid_STAR, errOut.str());
            exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
        };
        twoPass.yes = true;
    ...
    if (runMode == "alignReads" && pGe.gLoad != "Remove" && pGe.gLoad != "LoadAndExit")
    { // open reads files to check if they are present
        //        openReadsFiles();
        ...
        if (runMode == "alignReads" && outReadsUnmapped == "Fastx")
        { // open unmapped reads file
...
#define BUFFER_InSizeFraction 0.5
    if (limitIObufferSize < limitOutSJcollapsed * Junction::dataSize + 1000000)
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with larger --limitIObufferSize or smaller --limitOutSJcollapsed\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    for (uint imate = 0; imate < readNmates; imate++)
    {
        if (clip3pAdapterSeq.at(imate).at(0) == '-')
        { // no clipping
    ...
    else
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with allowed --waspOutputMode options: None or SAMtag\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    if (wasp.yes && !var.yes)
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with --waspOutputMode ... and --varVCFfile /path/to/file.vcf\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    if (wasp.yes && outSAMtype.at(0) != "BAM")
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with --waspOutputMode ... and --outSAMtype BAM ... \n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    {
        // modified in spatialTrans
        vAttr1 = {"NH", "HI", "AS", "nM"};
    }
    else if (outSAMattributes.at(0) == "spatial")
    {
        // modified in spatialTrans
        vAttr1 = {"NH", "HI", "AS", "nM", "Cx", "Cy", "UR"};
    }
    else
    {
    ...
    for (uint ii = 0; ii < vAttr1.size(); ii++)
    {
        ...
        else if (vAttr1.at(ii) == "XS")
        {
            ...
            {
                inOut->logMain << "WARNING --outSAMattributes contains XS, therefore BCSTAR will use --outSAMstrandField intronMotif" << endl;
                outSAMstrandField.type = 1;
            };
        }
        else if (vAttr1.at(ii) == "Cx")
        {
            outSAMattrOrder.push_back(ATTR_Cx);
            outSAMattrOrderQuant.push_back(ATTR_Cx);
            outSAMattrPresent.Cx = true;
        }
        else if (vAttr1.at(ii) == "Cy")
        {
            outSAMattrOrder.push_back(ATTR_Cy);
        ...
        else
        {
            ...
            errOut << "SOLUTION: re-run BCSTAR with --outSAMattributes that contains only implemented attributes\n";
            exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
        };
    };
    ...
    if (!var.yes && (outSAMattrPresent.vA | outSAMattrPresent.vG))
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with a --varVCFfile option, or without vA/vG tags in --outSAMattributes\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    if (!wasp.yes && outSAMattrPresent.vW)
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with a --waspOutputMode option, or without vW tags in --outSAMattributes\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    if (outSAMattrRG.size() > 0 && !outSAMattrPresent.RG)
    {
        ...
        outSAMattrOrderQuant.push_back(ATTR_RG);
        outSAMattrPresent.RG = true;
        inOut->logMain << "WARNING --outSAMattrRG defines a read group, therefore BCSTAR will output RG attribute" << endl;
    }
    else if (outSAMattrRG.size() == 0 && outSAMattrPresent.RG)
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with a valid read group parameter --outSAMattrRGline\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    {
        outSAMattrOrder.push_back(ATTR_XS);
        inOut->logMain << "WARNING --outSAMstrandField=intronMotif, therefore BCSTAR will output XS attribute" << endl;
    };
    ...
    if (wasp.yes && !outSAMattrPresent.vW)
    {
        ...
        outSAMattrPresent.vW = true;
        inOut->logMain << "WARNING --waspOutputMode is set, therefore BCSTAR will output vW attribute" << endl;
    };

    if (pSolo.type == 0 && (outSAMattrPresent.CR || outSAMattrPresent.CY || outSAMattrPresent.UY))
    {
        ostringstream errOut;
        errOut << "EXITING because of FATAL INPUT ERROR: --outSAMattributes contains CR/CY/UR/UY tags, but --soloType is not set\n";
        errOut << "SOLUTION: re-run BCSTAR without these attribures, or with --soloType set\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    else
    {
        ...
        errOut << "SOLUTION: re-run BCSTAR with --alignEndsType Local OR EndToEnd OR Extend5pOfRead1 OR Extend3pOfRead1\n";
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
        ...
        //             errOut <<"SOLUTION: run BCSTAR with    --genomeLoad NoSharedMemory    option\n";
        //             exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
        //         };
        //     #endif
...
// open compilation-dependent streams
    ...
    if (pGe.gLoad != "LoadAndKeep" && pGe.gLoad != "LoadAndRemove" && pGe.gLoad != "Remove" && pGe.gLoad != "LoadAndExit" && pGe.gLoad != "NoSharedMemory")
    { // find shared memory fragment
        ostringstream errOut;
    ...
    if (pGe.gLoad != "NoSharedMemory" && sjdbInsert.yes)
    {
        ...
        errOut << "SOLUTION: run BCSTAR with --genomeLoad NoSharedMemory to avoid using shared memory\n"
               << flush;
        sawErrCode(err_param_invalid_STAR, errOut.str());
        exitWithError(errOut.str(), std::cerr, inOut->logMain, EXIT_CODE_PARAMETER, *this);
    };
    ...
    if (outBAMcoord && limitBAMsortRAM == 0)
    { // check limitBAMsortRAM
        if (pGe.gLoad != "NoSharedMemory")
        {
            ostringstream errOut;
    ...
    else
    {
        ...
        for (uint ii = 0; ii < outSAMunmapped.mode.size(); ii++)
            errOut << " " << outSAMunmapped.mode.at(ii);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void Parameters::scanAllLines(istream &streamIn, int inputLevel, int inputLevelRequested)
{ // scan
...
int Parameters::scanOneLine(string &lineIn, int inputLevel, int inputLevelRequested)
{ // scan one line and load the parameters,
    ...
    if (inputLevel == 0 && (lineIn.substr(0, 1) == " " || lineIn.substr(0, 1) == "\t"))
        return 0; // for Default input spaces also mark comments, for nice formatting

    string parIn("");
    lineInStream >> parIn;
    if (parIn == "" || parIn.substr(0, 2) == "//" || parIn.substr(0, 1) == "#")
    ...
    lineInStream.str(lineIn);
    lineInStream.clear();
    lineInStream >> parIn; // get the correct state of stream, past reading parIn

    if (iPar == parArray.size())
    ...
    { // found the corresponding parameter
        if (inputLevel == 0 && parArray[iPar]->inputLevel > 0)
        { // this is one of the initial parameters, it was read from Command Line and should not be re-defined
        ...
        else if (parArray[iPar]->inputLevelAllowed > 0 && parArray[iPar]->inputLevelAllowed < inputLevel)
        { // this is initial parameter and cannot be redefined
            ostringstream errOut;
        ...
        else if (parArray[iPar]->inputLevel == inputLevel)
        { // this parameter was already defined at this input level
            ostringstream errOut;
        ...
        { // read values
            ...
            if (inOut->logMain.good())
            {
                ...
                if (parArray[iPar]->inputLevel > 0)
                    inOut->logMain << "     ~RE-DEFINED";
                inOut->logMain << endl;
            };
        };
    };
