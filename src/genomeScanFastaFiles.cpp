...
uint genomeScanFastaFiles (Parameters &P, char* G, bool flagRun, Genome &mapGen) {//scans fasta files. flagRun=false: check and find full size, flaRun=true: collect all the data
    ...
    for (uint ii=0;ii<mapGen.pGe.gFastaFiles.size();ii++) {//all the input files
        ...
        {//
            ...
            string errMsg="failed to open the file,"+mapGen.pGe.gFastaFiles.at(ii);
        ...
        {//
            ostringstream errOut;
            string errMsg="could not read from genomeFastaFile:"+mapGen.pGe.gFastaFiles.at(ii);
        ...
        {
            ostringstream errOut;
            string errMsg="EXITING because of INPUT ERROR: the file format of the genomeFastaFile: " +mapGen.pGe.gFastaFiles.at(ii) + " is not fasta:";
            errMsg+= " the first character is '" +to_string(cc)+"' ("+ to_string(cc+0) + "), not '>'.\n";
            errMsg+= " Solution: check formatting of the fasta file. Make sure the file is uncompressed (unzipped).\n";
