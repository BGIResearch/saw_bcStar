...
void ParametersChimeric::initialize(Parameters *pPin)
{
    ...
    for (const auto& type1 : out.type) {
        if (type1=="WithinBAM") {
            ...
            errOut <<"SOLUTION: re-run BCSTAR with --chimOutType Junctions , SeparateSAMold  , WithinBAM , HardClip \n";
    ...
    if (out.bam && !pP->outSAMattrPresent.NM) {
       ...
       pP->inOut->logMain << "WARNING --chimOutType=WithinBAM, therefore BCSTAR will output NM attribute" <<endl;
    };
